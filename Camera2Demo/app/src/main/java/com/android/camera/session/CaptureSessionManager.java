...
public interface CaptureSessionManager {
    ...
    interface SessionListener {
         ...
         * processed.
         */
        void onSessionQueued(Uri mediaUri);

        /**
         * Called when the media underlying the session with the given Uri has
         * been updated.
         */
        void onSessionUpdated(Uri mediaUri);
         ...
         */
        void onSessionCaptureIndicatorUpdate(Bitmap bitmap, int rotationDegrees);

        /** Called when the session with the given Uri finished. */
        void onSessionDone(Uri mediaUri);

        /** Called when the session with the given Uri failed processing. */
        void onSessionFailed(Uri mediaUri, int failureMessageId, boolean removeFromFilmstrip);

        /** Called when the session with the given Uri was canceled. */
        void onSessionCanceled(Uri mediaUri);

        /** Called when the session with the given Uri has progressed. */
        void onSessionProgress(Uri mediaUri, int progress);

        /** Called when the session with the given Uri has changed its progress text. */
        void onSessionProgressText(Uri mediaUri, int messageId);
         ...
         */
        void onSessionPictureDataUpdate(byte[] pictureData, int orientation);
    }
     ...
     */
    void putSession(Uri sessionUri, CaptureSession session);
     ...
     */
    CaptureSession removeSession(Uri sessionUri);
     ...
     * in-flight.
     */
    void fillTemporarySession(SessionListener listener);

    /**
     * Gets the directory to be used for temporary data. See
     * {@link SessionStorageManager#getSessionDirectory(String)}
     */
    File getSessionDirectory(String subDirectory) throws IOException;
    ...
    void putErrorMessage(Uri uri, int failureMessageId);
}
